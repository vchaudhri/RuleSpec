...
function queryOutputs() {
    ...
    //drop current input statusi
    var curs = compfinds('X', seq('current','X'), lambda, library);
    for (var i in curs) {
        dropfact(seq('current', curs[i]), lambda);
    }
    ...
    for (relation in getstrata(library).sort()) {
	    ...
	    console.log (relation)
            console.log(q);
            var result = tracefinds(q,q,lambda,library);
...
function updateCurrentInputs() {
    var input_seen = new Set();
    console.log('updating current inputs.');
    for (i in _trace_vars) { console.log(_trace_vars[i][0]) ;}
     
    for (i in _trace_vars) {
        var trace_type = _trace_vars[i][0];
        if (trace_type !== "call") continue;

        var p = _trace_vars[i][1];
	console.log('look up variable');
	console.log(p);
        var inputs = compfinds('X',read('input(X)'),lambda,library);
        if (inputs.indexOf(p[0]) > -1) {
          var cur = compfindx(seq('current',p[0]), seq('current',p[0]), lambda, library);
          var existing = compfindx(seq(p[0],'X'), seq(p[0],'X'), lambda, library);
            if (existing /*|| result.length > 0*/) {
		console.log('dropping fact');
		console.log(cur) ;
              dropfact(cur, lambda);
          }
          else if (!input_seen.has(p[0])) {
              console.log('Need input for: ' + p[0]);
              _debug.push('Need input for: ' + p[0]);
              /*input_val = prompt("Enter the value of " + p[0]);
              newp = seq();
              newp.push(p[0]);
              newp.push(input_val);
              savefact(newp, lambda);*/

              //dropfact(cur, lambda);
              savefact(seq('current',p[0]), lambda);
    ...
    if (typeof refresh === 'function') {
        updatemenuoptions();
        refresh();
    } else if (typeof populatesheet === 'function') {
        populatesheet();
    }
    _trace_vars = [];
}

function generateWorksheet(clean = true) {
    var worksheet = "";

    if (clean) {
        library = devalueify(lambda,library);
        library = desameify(lambda,library);
    }

    var worksheetParts = generateWorksheetHtml(lambda,library);


    for (i in worksheetParts) {
        worksheet += worksheetParts[i];
        worksheet += '\n';
    }
    return worksheet;
}

function generateWorksheetHtml(database, ruleset) {

    var worksheetHtml = seq('<html>', '<head>', '<title></title>', '<meta charset="UTF-8">', '<style class="worksheet">',
    '[type="widget"] > *',
    '{ min-height: 2px; }',
    '</style>', '<script src="/epilog/javascript/epilog.js"></script><script src="../javascript/worksheets.js"></script>',
    '</head>',
    '<body onload="initialize(); queryOutputs();" style="width: 100%; overflow: auto; margin: 0px; box-sizing: border-box;">',
    '<pre id="lambda" style="display:none;" type="text/hrf">',
    grindem(database),
    '</pre>',
    '<pre id="library" style="display:none;" type="text/hrf">',
    grindem(ruleset));

    uiRules = generateUIrules(database, ruleset);
    worksheetHtml = worksheetHtml.concat(uiRules);

    worksheetHtml.push('</pre>');

    widgetHtml = generateWidgetHtml(database, ruleset);
    worksheetHtml = worksheetHtml.concat(widgetHtml);

    worksheetHtml.push('</body>'); 
    worksheetHtml.push('</html>'); 

    return worksheetHtml;
}

function generateUIrules(database, ruleset) {
    var ui_rules = seq("%%ui scaffolding%%");
    var q = seq('input','X');
    var inputs = compfinds('X',q,database,ruleset);

    var y_coord = 50;
    for (var i in inputs) {
        var select1 = seq('transition', seq('select', inputs[i], 'X'), seq(inputs[i], 'X') );
        var select2 = seq('transition', seq('and', seq('select', inputs[i], 'X'), seq(inputs[i], 'Y') ), seq('not', seq(inputs[i], 'Y') ) );
        var value1 = seq('rule', seq('value', inputs[i], 'X'), seq(inputs[i], 'X') );
        var yoff1 = seq('rule', seq('attribute', seq('text', inputs[i]), 'yoffset', y_coord.toString()), seq('input', inputs[i]) );
        var insert_row = 'insertrow(input_table(' + inputs[i]  + ',X)) ==> ' + inputs[i] + '(X)';
        var delete_row = 'deleterow(input_table(' + inputs[i] + ',X)) ==> ~' + inputs[i] + '(X)';
        var update_row = 'updaterow(input_table(' + inputs[i] + ',X), input_table(' + inputs[i] + ',Y)) ==> ~' + inputs[i] + '(X) & ' + inputs[i] + '(Y)';

        ui_rules.push(grind(select1));
        ui_rules.push(grind(select2));
        ui_rules.push(grind(value1));
        ui_rules.push(grind(yoff1));
        ui_rules.push(grind(insert_row));
        ui_rules.push(grind(delete_row));
        ui_rules.push(grind(update_row));
        
        y_coord += 20;
    }
    
    var output_names = compfinds('X',seq('output','X'),database,ruleset);
   
    for (var i in output_names) {
        var value_out = seq('rule', seq('value', output_names[i], 'X'), seq(output_names[i], 'X') );
        
        ui_rules.push(grind(value_out));
    }

    //static rules
    var style1 = seq('rule', seq('style', 'X', '"display"', '"none"'), seq('input', 'X'), seq('not', seq('current', 'X') ) );
    var style2 = seq('rule', seq('style', 'X', '"display"', '""'), seq('input', 'X'), seq('current', 'X') );
    var style_text1 = seq('rule', seq('style', seq('text', 'X'), '"display"', '"none"'), seq('input', 'X'), seq('not', seq('current', 'X') ) );
    var style_text2 = seq('rule', seq('style', seq('text', 'X'), '"display"', '""'), seq('input', 'X'), seq('current', 'X') );
    var xref = 'attribute(X, xref, Xs) :- input(X) & stringify(text(X), Xs)';
    var yref = 'attribute(X, yref, Xs) :- input(X) & stringify(text(X), Xs)';
    var xoff2 = 'attribute(X, xoffset, right) :- input(X)';
    var yoff2 = 'attribute(X, yoffset, center) :- input(X)';
    var xoff1  = 'attribute(text(X), xoffset, 100) :- input(X)';
    var input_text = 'innerhtml(text(X), Str) :- input(X) & stringify(X,Xs) & stringappend(Xs, ": ", Str)';
    var output_table = 'output_table(X,Y) :- output(X) & value(X,Y)';
    var input_table = 'input_table(X,Y) :- input(X) & value(X,Y)';
    y_coord += 50;
    var yoff_table = 'attribute(output_table, yoffset,' + y_coord.toString()  + ')';
    
    ui_rules.push(grind(style1));
    ui_rules.push(grind(style2));
    ui_rules.push(grind(style_text1));
    ui_rules.push(grind(style_text2));
    ui_rules.push(xref);
    ui_rules.push(yref);
    ui_rules.push(xoff1);
    ui_rules.push(xoff2);
    ui_rules.push(yoff2);
    ui_rules.push(input_text);
    ui_rules.push(output_table);
    ui_rules.push(input_table);
    ui_rules.push(yoff_table);

    ui_rules.push("%%end ui scaffolding%%");
    return ui_rules;
}

function generateWidgetHtml(database, ruleset) {
    var widget_html = seq();
    ...
    for (var i in inputs) {
        var text_input = '<input type="text" onchange="modtext(this); queryOutputs();" id="' + inputs[i] + '" data-x="NaN" data-y="NaN" widget="objectfield" style="border: 1px solid lightgrey; outline: 0px; box-sizing: border-box; margin: 0px; position: absolute; display: none; border-radius: 0px; -moz-border-radius: 0px; -webkit-border-radius: 0px;" xoffset="right" yoffset="center" class="" positioned="true" xref="" yref="">';
        var display_text = '<div spellcheck="false" autocomplete="false" id="text(' + inputs[i] + ')" data-x="100" data-y="50" widget="div" style="background-color: white; box-sizing: border-box; flex-direction: column; text-align: left; justify-content: flex-start; overflow: auto; margin: 0px; position: absolute; display: none; border-radius: 0px; -moz-border-radius: 0px; -webkit-border-radius: 0px;"  xref="" yref="" class="" positioned="true"></div>';
    
        widget_html.push(text_input);
        widget_html.push(display_text);
    }

    var output_table = '<table border="1" cellspacing="0" arity="2" class="data-table" id="output_table" editable-row-color="lightblue" header-background="#333" header-color="#EEE" header-alignment="center" widget="table" data-x="35.7735" data-y="324.342375" style="width: 200px; background-color: white; transform: translate(35.7735px, 324.342px); position: absolute; border-radius: 0px; -moz-border-radius: 0px; -webkit-border-radius: 0px;" xoffset="50" yoffset="" xref="" yref="" positioned="true"><caption><strong>Output</strong></caption><thead><tr style="font-weight: bold; background-color: rgb(51, 51, 51); color: rgb(238, 238, 238); text-align: center;"><td type="object">Name</td><td type="object">Value</td></tr></thead><tbody></tbody></table>';

    var input_table = '<table onclick="queryOutputs();" border="1" cellspacing="0" arity="2" class="data-table" editable="true" id="input_table" editable-row-color="lightblue" header-background="#333" header-color="#EEE" header-alignment="center" widget="table" data-x="335.77349853515625" data-y="324.3423767089844" style="width: 200px; background-color: white; transform: translate(335.773px, 324.342px); position: absolute; border-radius: 0px; -moz-border-radius: 0px; -webkit-border-radius: 0px;" xoffset="300" yoffset="0" xref="output_table" yref="output_table" positioned="true"><caption><strong>Inputs</strong></caption><thead><tr style="font-weight: bold; background-color: rgb(51, 51, 51); color: rgb(238, 238, 238); text-align: center;"><td type="object">Name</td><td type="object">Value</td><td></td><td></td></tr></thead><tfoot><tr style="text-align: center; color: rgb(51, 51, 51); background-color: rgb(238, 238, 238); padding-top: 10px;"><td colspan="4"><strong onclick="addtablerow(this.parentNode)" style="cursor: pointer;">&#10010; Add row</strong></td></tr></tfoot><tbody></tbody></table>';

    widget_html.push(output_table);
    widget_html.push(input_table);
    
    return widget_html;
}

function devalueify(database, ruleset) {
    var new_ruleset = ruleset.slice();

    var q = seq('input','X');
    var inputs = compfinds('X',q,database,ruleset);

    var q = seq('output','X');
    var outputs = compfinds('X',q,database,ruleset);

    var cat = inputs.concat(outputs);
    var IOs = new Set(cat);
    
    for (var i in ruleset) {
        for (var j in ruleset[i]) {
            if (ruleset[i][j] instanceof Array && ruleset[i][j][0] === 'value') {
                if (IOs.has(ruleset[i][j][1])) {
                    //rewrite value(io, v) as io(v)
                    var new_rule = seq(ruleset[i][j][1], ruleset[i][j][2]);
                    new_ruleset[i][j] = new_rule;
                }
            }
        }
    }
    ...
    var newlib =  seq();
    definetheory(newlib, new_ruleset);
    return newlib;
}

function desameify(database, ruleset) {
    var new_ruleset = ruleset.slice();
    ...
    for (var i in ruleset) {
        ...
        for (var j in ruleset[i]) {
            if (ruleset[i][j] instanceof Array && ruleset[i][j][0] === 'same') {
                var theta = ruleset[i][j][1];
                var substitution = ruleset[i][j][2];
                //substitute the entire rule for every occurrence of theta
                var new_rule = applySubstitution(new_ruleset[i], theta, substitution);
                sameRules.push(j);
                new_ruleset[i] = new_rule;
            }
        }

        var offset = 0;
        for (var idx in sameRules) {
                new_ruleset[i].splice(sameRules[idx] - offset, 1);
                offset += 1;
        }
    }
...
function applySubstitution(exp, th, s) {
    var nexp = exp.slice();
    if (exp === th) {
        return s;
    }
    ...
    if (exp instanceof Array) {
        for (var i in exp) {
            nexp[i] = applySubstitution(exp[i], th, s);
        }
    }
